...
def main(data_folder,saved_model_folder):
    ...
    for epoch in range(model_flag, epochs):
        ...
        if epoch + 1 in [5, 10, 20, 50, 100, 200, 500]:
            torch.save(model.state_dict(), os.path.join(saved_model_folder, 'pebg_%d.ckpt' % (epoch + 1)))
    ...
    np.savez(os.path.join(data_folder, 'embedding_%d.npz' % epochs), pro_repre=pro_repre,
...
if __name__ == '__main__':
    ...
    main(data_folder, saved_model_folder)
