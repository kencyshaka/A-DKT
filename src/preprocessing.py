...
if not os.path.isdir("../data/DKTFeatures_"+str(config.assignment)):
    os.mkdir("../data/DKTFeatures_"+str(config.assignment))

np.save("../data/DKTFeatures_"+str(config.assignment)+"/training_students.npy", train_val_s)
...
for fold in range(10):
    ...
    file_train = open("../data/DKTFeatures_"+str(config.assignment)+"/train_firstatt_"+str(fold)+".csv","w")
    for s in train_s:
        if d[s]['length']>0:
    ...
    file_val = open("../data/DKTFeatures_"+str(config.assignment)+"/val_firstatt_"+str(fold)+".csv","w")
    for s in val_s:
        if d[s]['length']>0:
