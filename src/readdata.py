...
class data_reader():
    ...
    def get_data(self, file_path):
        ...
        if config.assignment == 487:
            np.save("../data/DKTFeatures_"+str(config.assignment)+"/np_counts_"+str(self.fold )+".npy", [node_count, path_count])
        else:
            np.save("../data/DKTFeatures_"+str(config.assignment)+"/np_counts.npy", [node_count, path_count])
