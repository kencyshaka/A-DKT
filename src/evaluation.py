...
import wandb
def performance_granular(batch, pred, ground_truth, prediction, epoch, config,fold):
        ...
        for i in range(len(p)):
            for j in range(config.questions):
                if delta[i,j] == 1:              #student attempted this question and add their prediction and ground thruth
                    preds[j].append(p[i])
                    gts[j].append(a[i])
                    if i == 0 or delta[i-1,j] != 1:   #for that attempt check if the previous is not 1, i.e its a first attempt
    ...
    # columns = ["fold","auc", "f1", "recall", "precision","acc"]
    # table_data = []
    # first_table_data = []
    ...
    for j in range(config.questions):
        ...
        # table_data.append([fold,scores[j][0], scores[j][1], scores[j][2], scores[j][3], scores[j][4]])
        ...
        print('First prediction for problem '+str(j)+' auc: ' + str(first_auc) + ' f1: ' + str(first_f1) + ' recall: '
              + str(first_recall) + ' precision: ' + str(first_precision) + ' acc: ' + str(first_acc))
...
def train_epoch(model, trainLoader, optimizer, loss_func, config, device,epoch):
    ...
    for batch in tqdm.tqdm(trainLoader, desc='Training:    ', mininterval=2):
        ...
        pred = model(batch_new)                  # trains the model and get the results [batch, 49,10]
...
def test_epoch(model, testLoader, loss_func, device, epoch, config, fold):
    ...
    return performance_granular(full_data, preds, ground_truth, prediction, epoch, config,fold)
